...
A 3D restaurant service simulator and a foundation for testing low-latency decision models. Built with TypeScript, Three.js, and Vite. Rules modes run locally without an API key; real Jev (TypeSafe) and an optional legacy OpenAI stand-in use separate API keys.

**Real Jev is connected through the TypeSafe HTTP API.** Camera-assisted rules remain a deterministic baseline. Jev and the legacy OpenAI stand-in hold simulation time during requests. These controlled benchmarks compare decision quality; measured network latency is reported separately and is not charged to simulated diners. They do not establish real-world restaurant performance.
...
The adapter calls `POST https://api.typesafe.ai/v1/systemone` with a named JSON state and one Choice question. It constructs one single-stop option and, when useful, one multi-stop option for every feasible first action (at most 200 alternatives from the 100-candidate local API limit; the documented Choice limit is 255). Follow-ups use the baseline's discounted-work-per-time objective, with a bounded search of up to three stops. No first action is removed to force a preferred answer. This is a bounded choice architecture, not exhaustive planning or generated dialogue. Jev's choice is mapped back to original candidate IDs; route distances, projected dirty load, water, terminal inventory stops, reservations and stale action validation stay in code.
...
Private repository on GitHub. Choose a license before any public release. Public performance claims require controlled multi-seed evaluations and real-world calibration; a working API connection alone is not validation.
